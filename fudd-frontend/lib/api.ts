...
export const API_BASE_URL =
    process.env.NEXT_PUBLIC_API_BASE_URL || 'https://fudd-backend.onrender.com';
...
export async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
    const token =
        typeof document !== 'undefined'
            ? document.cookie
                  .split('; ')
                  .find((row) => row.startsWith('fudd-auth-token='))
                  ?.split('=')[1]
            : null;
...
export async function apiBacktest(input: BacktestInput): Promise<BacktestOutput> {
    return apiFetch<BacktestOutput>('/api/v1/backtest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(input),
    });
